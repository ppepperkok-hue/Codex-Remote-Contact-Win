...
function shouldAck(text) {
  ...
  return /(打开|启动|运行|开启|关闭|停止|清理|清空|删除|下载|安装|搜索|查找|查询|查看|检查|检测|修复|设置|配置|操作|点击|执行|运行|处理|分析|对比|总结|翻译|整理|放歌|播放|截图|截屏|写|做个|帮我|弄一下|搞定|优化|安装|更新|备份|压缩|解压|上传|发送)/i.test(
    String(text || "")
  );
}
